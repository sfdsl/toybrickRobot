...
## S7. 复扫 / 重扫（提高地图质量，约 20~30 分钟）

**为什么要重扫**：地图里"未知区（灰）+ 零星假障碍（噪点）"这两样直接决定导航能力——
未知区**不可通行**、噪点膨胀后会把通道切碎，于是只能把 `--inflation` 从 0.25 降到 0.15 才勉强走得通。
有没有变好，**用数字说话**（`mapq` 报告）。

**当前地图基线（`mapq` 口径，2026-09-21 11:46 重扫后——旧图已被替换）**：

| 指标 | 当前值（新图 `robot_map`） | 目标 |
|---|---|---|
| 未知区占比 | **59.7%**（包围盒 12.60 m × 15.05 m，含门外/走廊未扫区） | ≤ 20% |
| 膨胀 0.25 m 下可通行 | 11.3%（与起点连通 88.8%） | ≥ 25% |
| 膨胀 0.15 m 下可通行 | 18.4%（连通 95.2%），**绝对可用面积 ≈ 35 m²** | — |
| 小障碍团簇（<4 格） | 270 处 / 403 格 —— **椅腿等真实障碍，保留** | 不设目标 |
| 起点 | (0,0) 在空闲区且 0.15 m 膨胀下可通行 ✓（标注图 `robot_map_start.png`） | — |

> **教训（2026-09-21）**：上一版地图（213×232，未知 44.7%、可用 ≈37 m²）在重扫时被直接覆盖，
> 而 `04_map/` 在 `.gitignore` 里、也没有别处备份 → **已不可恢复**。
> 以后重扫前先把旧四件套复制到 `04_map/archive_<日期>/`，两张图各自跑 `mapq` 对比后再决定淘汰谁。

> ⚠ **2026-09-21 更正**：早前把图上的小点当"SLAM 噪点"并默认抹掉（`--min-obstacle-cells 4`）。
> 实测确认那些是**椅子腿**——抹掉等于让车从椅子中间穿过去。现在默认 **0（一个都不抹）**；
> 诚实口径下 4 个测试目标 3 个可达（路径 8.02 / 不可达 / 3.03 / 6.62 m）。
> **所以重扫的目标不是"减少小点"，而是"减少未知区"。**

**步骤**：

1. **先决定环境，再保持前后一致（比"清场"更重要）**：
   - 导航/演示时**椅子会在** → 就按原位摆好扫进去（它们是真实障碍，路径会绕开它们）
   - 导航时场地**会清空** → 就把椅子、箱子、地上的线缆都搬出去再扫（扫出来就是空场地）
   - 人也别站在雷达扫得到的地方"临时留个影子"——**那**才是真正的噪点
2. **标记起点（最省事的一步，强烈建议）**：地上贴美纹纸标出**四轮位置**，再画一条**车头方向的箭头**。
   新图的坐标系原点 = 开局那一刻的车位与朝向（即 `map_start_pose [0,0,0]` 的含义）——
   标了它，以后 `robotnav` 定位就照标记摆车，不用再像 N1 那样倒推像素
3. 起建图：`robot`（mapping 模式）；另开 `mapv`，或手机 `phone` 看实时图
4. **路线（建议顺序，速度 0.15 m/s）**：
   - `drive spin` —— 出发前原地转一圈，先把四周扫进来
   - **沿墙绕一整圈（回环！）**：没有避障，所以**分段走、每段 ≤2 m**，人跟着看，如
     `drive -p "f2;t90;f2;t90;f2;t90;f2;t-90;"`（按现场实际情况调整）
   - 内部**弓字形补扫**：`drive grid -L 2 -W 0.8 -n 3`（行距 ≥ 车宽 0.24 m + 余量）
   - 回到起点附近再 `drive spin` —— 强化回环闭合
   - 剩下的灰区用手机遥控（`phone`）看着实时图一个个开进去补
5. **别做**：横移（麦轮里程计不可信）、快速急转、贴着墙 <0.2 m 走
6. 保存：`savemap`（自动带时间戳 → `04_map/map_YYYYmmdd_HHMMSS.*`；**旧图保留**，别覆盖）
7. 复验：`mapq --map ~/RobotCode/04_map/<新前缀>` → 三项判据都达标才算扫好；
   再用 `navcheck --map <新前缀> --inflation 0.25 --goals "..."` 确认路径离墙距离

**换用新图（导航/定位）**：

- 定位加载**建图序列**（`.posegraph`/`.data`）：把 `mapper_params_localization.yaml` 的
  `map_file_name` 指到新前缀（`src` 与 `install` 两份同步），或把新四件套改名覆盖 `robot_map.*`
- 规划读 `pgm/yaml`：`nav --map ~/RobotCode/04_map/<新前缀>`
- 换图后**必须**：按新起点标记摆车 → `robotnav` → `mapv` 确认车标与实际重合 → 再跑导航

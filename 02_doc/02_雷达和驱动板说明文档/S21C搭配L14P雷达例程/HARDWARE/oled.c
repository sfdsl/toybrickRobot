...
extern u16 receive_cnt2,receive_cnt3,receive_cnt4,receive_cnt5;//计算成功接收数据帧次数
extern LidarPointStructDef Dataprocess2[800];
extern LidarPointStructDef Dataprocess3[800];
extern LidarPointStructDef Dataprocess4[800];
extern LidarPointStructDef Dataprocess5[800];
extern u16 data_cnt2;
extern u16 data_cnt3;
extern u16 data_cnt4;
extern u16 data_cnt5;
/**************************************************************************
...
//	memset(OLED_GRAM,0, 128*8*sizeof(u8));	//GRAM清零但不立即刷新，防止花屏
	/*     第一行串口2测距距离显示     */
		OLED_ShowString(0,0,"Dist1:");
		OLED_ShowNumber(70,00,Lidar_Distance[0],5,12);				//显示雷达1在角度范围内的距离值
		OLED_ShowString(110,0,"mm");
  /*     第二行串口3测距距离显示     */
		OLED_ShowString(0,10,"Dist2:");	
		OLED_ShowNumber(70,10,Lidar_Distance[1],5,12);				//显示雷达2在角度范围内的距离值
		OLED_ShowString(110,10,"mm");
	/*     第三行串口4测距距离显示     */
		OLED_ShowString(0,20,"Dist3:");
		OLED_ShowNumber(70,20,Lidar_Distance[2],5,12);				//显示雷达3在角度范围内的距离值
		OLED_ShowString(110,20,"mm");
	/*     第四行串口5测距距离显示     */
		OLED_ShowString(0,30,"Dist4:");	
		OLED_ShowNumber(70,30,Lidar_Distance[3],5,12);				//显示雷达4在角度范围内的距离值
		OLED_ShowString(110,30,"mm");
	/* 		 第五行显示功接受雷达数据包的帧数计数值*/
		OLED_ShowString(0, 40, "Range:");
		OLED_ShowNumber(50, 40,range_Left, 3, 12);						//显示角度范围的左值
		OLED_ShowString(70, 40, "-");
		OLED_ShowNumber(80, 40,range_Right, 3, 12);						//显示角度范围的右值
	/* 		 第六行显示功接受雷达数据包的帧数计数值*/
		if(USE_CAN_FLAG)
			OLED_ShowString(0, 50, "CAN  Lidar_N:");
		else
			OLED_ShowString(0, 50, "Lidar_Number:");
		OLED_ShowNumber(100,50, Lidar_1+Lidar_2+Lidar_3+Lidar_4,2,12);	//接入的雷达数量
		//如果使用CAN的话则串口不打印数据
		if(!USE_CAN_FLAG)
			printf("angle_range:%d - %d\r\nliar1:%dmm  liar2:%dmm  liar3:%dmm  liar4:%dmm\r\n\r\n",range_Left,range_Right,Lidar_Distance[0],Lidar_Distance[1],Lidar_Distance[2],Lidar_Distance[3]);
